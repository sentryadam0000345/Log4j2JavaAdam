...
public class Application {
    ...
    @RequestMapping("/errorgeneratorone")
    @ResponseBody
    String errorGeneratorone() {

        // create an array of Strings

        //int ArrayOfHashCodes[] = new int[] {1186874997, 1186874998,1186874996};
        int ArrayOfHashCodes[] = new int[] {1186874997};

        int index = (int)(2.0 * Math.random());

        String hashcode = Integer.toString(ArrayOfHashCodes[0]);

        try {
            int example = 1 / 0;
        } catch (Exception e) {
            // Add extra data to future events in this context.
            Sentry.getContext().addExtra("Hashcode", hashcode);
    ...
    @RequestMapping("/errorgeneratorthree")
    @ResponseBody
    String errorGeneratorthree() {
    ...
    @RequestMapping("/errorgeneratorfour")
    @ResponseBody
    String errorGeneratorfour() {
        ...
        String hashcode = Integer.toString(ArrayOfHashCodes[1]);
        ...
        try {
            ...
            EventBuilder eventBuilder = new EventBuilder()
                    .withSentryInterface(new MessageInterface("{} failed to provide an email address", "User43"))
                    //.withMessage("<cite>{} failed to provide an email address</cite>", "User1")
            ...
            //logger.error(hashcode, e);
        }
        return "Success";

    }

    @RequestMapping("/errorgeneratortwo")
    @ResponseBody
    String errorGeneratortwo() {
        ...
        String hashcode = Integer.toString(ArrayOfHashCodes[2]);
        ...
        try {
            ...
            EventBuilder eventBuilder = new EventBuilder()
                    .withSentryInterface(new MessageInterface("{} failed to provide an email address", "User1"))
                    //.withMessage("<cite>{} failed to provide an email address</cite>", "User1")
                    .withLevel(Event.Level.ERROR)
                    .withSentryInterface(new ExceptionInterface(e));
    ...
    private static void initSentry() {
        ...
        Sentry.getStoredClient().addShouldSendEventCallback(new ShouldSendEventCallback() {
            ...
            public boolean shouldSend(Event event) {
                ...
                System.out.print("EVENT " + event.getExtra().get("Hashcode"));
                System.out.print("EVENT " + event.getContexts());
                System.out.print("EVENT " + event.getMessage());
                System.out.print("GETTTERZ ");


                List<String> list = new ArrayList<String>();
                list.add(event.getMessage());
                //list.add(Integer.toString((int) event.getExtra().get("Hashcode")));
                if (event.getExtra().containsKey("Hashcode")) {
                    list.add(event.getExtra().get("Hashcode").toString());
